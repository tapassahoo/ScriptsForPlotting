...
distance_flag = True
plot_type = "first excited state energy"
...
rlist = np.arange(8.0, 10.01, 1.0, dtype=float)
		...
		if ((parameter_name == "beta") and (distance_flag == True)):
			...
			if (plot_type == "order_parameter"):
				generator.get_plot_order_parameter_vs_rpt(
						...
						energy_per_neighbours,
						purpose)
			if (plot_type == "first excited state energy"):
				generator.get_plot_first_excited_state_energy_vs_rpt(
						final_result_path,
						method,
						molecular_system,
						rotor,
