...
def FigureENT(TypeCal, molecule_rot, TransMove, RotMove, variableName, Rpt, dipolemoment, parameterName, parameter, numbblocks, numbpass, numbmolecules, molecule, ENT_TYPE, preskip1, postskip1, extra_file_name, src_dir, particleA, TypePlot, beadsRef):
	...
	if (((TypePlot == "RFACTOR") or (TypePlot == "GFACTOR")) and variableName == "tau"):
		...
		var = beadsRef
		FilePlotName = support.GetFileNamePlot(TypeCal, molecule_rot, TransMove, RotMove, variableName, Rpt, -1.0, -1.0, parameterName,
...
#
		if (plotnum != 2):
			plt.axhline(y=math.log(2.0), color='blue',
						lw=2.0, linestyle='-', label='ln(2)')
#
		if (plotnum != 2):
			nn = [2, 4, 6]
		else:
		...
		for numbmolecules in nn:
			...
			if (numbmolecules == 6):
				...
				numbblocks = 20000
			if (numbmolecules == 8):
				DList = [0.9106, 1.1152, 1.2878, 1.4398,
						 1.5772, 1.7036, 1.8212, 1.9317, 2.0362]
			...
			for dipolemoment in DList:
				...
				if (dipolemoment > 4.0):
					if (numbmolecules == 2):
						...
						numbblocks = 20000
				FilePlotName = support.GetFileNamePlot(TypeCal, molecule_rot, TransMove, RotMove, variableName, Rpt, -1.0, dipolemoment, parameterName,
				...
				beads1, var1, purity1, entropy1, err_purity1, err_entropy1 = genfromtxt(
					FileToBePlotEntropy, unpack=True, usecols=[0, 1, 4, 5, 8, 9], skip_header=preskip, skip_footer=postskip)
				if (numbmolecules <= 6):
			...
			print(entropy3Plot)
#
			plotEntropyENT1(numbmolecules, RFactorPlot, entropy1Plot, err_entropy1Plot,
							variableName, RFactorPlot, entropy2Plot, entropy3Plot, font, TypePlot)
		ymin, ymax = plt.ylim()
		xmin, xmax = plt.xlim()
		if plotnum == 1:
		...
		Text2 = ""
		if Text1:
			PlotLabel(Text1, Text2, font, xmin, xmax, ymin, ymax, variableName,
					  parameter, numbmolecules, molecule, Rpt, dipolemoment)

		plt.subplots_adjust(top=0.97, bottom=0.14, left=0.14,
							right=0.98, hspace=0.0, wspace=0.0)
		if (plotnum != 2):
			plt.legend(bbox_to_anchor=(0.68, 0.75), loc=2,
					   borderaxespad=1., shadow=True, fontsize=fontlegend)
		else:
			plt.legend(bbox_to_anchor=(0.98, 0.35), borderaxespad=1.,
					   shadow=True, fontsize=fontlegend)
...
##
			...
			entropy3 = 0.0
###================data reading begin==================###
			var1, purity1, entropy1, err_purity1, err_entropy1 = genfromtxt(
				FileToBePlot, unpack=True, usecols=[1, 4, 5, 8, 9], skip_header=preskip, skip_footer=postskip)
			if (numbmolecules <= 4):
				beads, entropy2 = genfromtxt(FileToBePlotMM, unpack=True, usecols=[
											 0, 2], skip_header=preskip, skip_footer=postskip)
				if (variableName == "tau"):
...
###================data reading end====================###
			...
			iFrame = iFrame + 1
			plotEntropyENT(var1, entropy1, err_entropy1,
						   variableName, var2, entropy2, entropy3, font, font)
			plt.ylabel(r'$S_{2}$', fontsize=font)
			ymin, ymax = plt.ylim()
			xmin, xmax = plt.xlim()
			...
			PlotLabel(Text1, Text2, font, xmin, xmax, ymin, ymax, variableName,
					  parameter, numbmolecules, molecule, Rpt, dipolemoment)
			plt.legend(bbox_to_anchor=(0.80, 0.50), loc=2,
					   borderaxespad=0., shadow=True, fontsize=fontlegend)
			plt.subplots_adjust(top=0.98, bottom=0.16,
								left=0.18, right=0.95, hspace=0.0, wspace=0.)
...
#
		...
		for ENT_TYPE in TypeList:
			...
			for dipolemoment in DList:
				...
				FilePlotName = support.GetFileNamePlot(TypeCal, molecule_rot, TransMove, RotMove, variableName, Rpt, dipolemoment, parameterName, parameter,
													   numbblocks, numbpass, numbmolecules, molecule, ENT_TYPE, preskip1, postskip1, extra_file_name, src_dir, particleA, beadsRef1)
				FileToBePlotEntropy = FilePlotName.SaveEntropy+".txt"
				...
				if (ENT_TYPE == "SWAPTOUNSWAP"):
					...
					beads1, var1, purity1, entropy1, err_purity1, err_entropy1 = genfromtxt(
						FileToBePlotEntropy, unpack=True, usecols=[0, 1, 4, 5, 8, 9], skip_header=preskip, skip_footer=postskip)
					entropyED = loadtxt(
						FileToBePlotED, unpack=True, usecols=[2])
				if (ENT_TYPE == "BROKENPATH"):
					beads1, var1, entropy1, err_entropy1 = genfromtxt(FileToBePlotEntropy, unpack=True, usecols=[
																	  0, 1, 4, 7], skip_header=preskip, skip_footer=postskip)
...
#
			...
			if (ENT_TYPE == "SWAPTOUNSWAP"):
				plt.plot(RFactorPlot, entropy2Plot, color='black', ls='-',
						 linewidth=1,  marker='o', markersize=9, label='ED')
			plt.errorbar(RFactorPlot, entropy1Plot, yerr=err_entropy1Plot, color=colorList[labelIndex], ls=lsList[
						 labelIndex], linewidth=1,	marker=markerList[labelIndex], markersize=8, label=labelList[labelIndex])
			...
			plt.yticks(np.arange(0.0, 0.91, step=0.1),
					   fontsize=font, rotation=0)

		plt.ylabel(r'$S_{2}$', fontsize=font)
		plt.xlabel(r'$g$', fontsize=font, labelpad=-3)

		plt.subplots_adjust(top=0.97, bottom=0.14, left=0.14,
							right=0.98, hspace=0.0, wspace=0.4)
		plt.legend(bbox_to_anchor=(0.40, 0.45), loc=2,
				   borderaxespad=1., shadow=True, fontsize=fontlegend)
		plt.savefig(outfileEntropy, dpi=200, format='eps')
...
def plotEntropyENT1(numbmolecules, var, val, err_val, variableName, var1, val1, val2, font, TypePlot):
	if (numbmolecules == 2):
		plt.errorbar(var, val, yerr=err_val, color="red", ls='-',
					 linewidth=1,  marker="o", markersize=8, label='PIGS: '+r'$N=2$')
		plt.plot(var, val2, color='black', ls='None', linewidth=1,
				 marker="o", markersize=10, label='ED: N=2')
	...
	if (numbmolecules == 4):
		plt.errorbar(var, val, yerr=err_val, color="red", ls='-',
					 linewidth=1,  marker="s", markersize=8, label='PIGS: N=4')
		plt.plot(var, val2, color='black', ls='None', linewidth=1,
				 marker="s", markersize=10, label='ED: N=4')
		#plt.plot(DMRGdatagFac, DMRGdataPlot, linestyle = 'None', color = 'darkcyan', marker = "s", markersize = 8, label = 'DMRG: 4 HF')

	if (numbmolecules == 6):
		plt.errorbar(var, val, yerr=err_val, color="red", ls='-',
					 linewidth=1,  marker="v", markersize=8, label='PIGS: N=6')
		plt.plot(var, val2, color='black', ls='None', linewidth=1,
				 marker="v", markersize=10, label='ED: N=6')
		#plt.plot(DMRGdatagFac, DMRGdataPlot, linestyle = 'None', color = 'darkcyan', marker = "8", markersize = 7, label = 'DMRG: 6 HF')

	if (numbmolecules == 8):
		plt.errorbar(var, val, yerr=err_val, color="red", ls='-',
					 linewidth=1,  marker="p", markersize=8, label='PIGS: N=8')
		#plt.plot(var, val2, color = 'black', ls = 'None', linewidth=1, marker = "v", markersize = 10, label = 'ED: N=6')
		#plt.plot(DMRGdatagFac, DMRGdataPlot, linestyle = 'None', color = 'darkcyan', marker = "8", markersize = 7, label = 'DMRG: 6 HF')

	if (numbmolecules == 16):
		plt.errorbar(var, val, yerr=err_val, color="blue", ls='-',
					 linewidth=1,  marker="8", markersize=8, label='PIGS: '+r'$N=16$')
		#plt.plot(var, val2, color = 'black', ls = 'None', linewidth=1, marker = "v", markersize = 10, label = 'ED: N=6')
		#plt.plot(DMRGdatagFac, DMRGdataPlot, linestyle = 'None', color = 'darkcyan', marker = "8", markersize = 7, label = 'DMRG: 6 HF')

	if (numbmolecules == 32):
		plt.errorbar(var, val, yerr=err_val, color="green", ls='-',
					 linewidth=1,  marker="^", markersize=8, label='PIGS: '+r'$N=32$')
		#plt.plot(var, val2, color = 'black', ls = 'None', linewidth=1, marker = "v", markersize = 10, label = 'ED: N=6')
		#plt.plot(DMRGdatagFac, DMRGdataPlot, linestyle = 'None', color = 'darkcyan', marker = "8", markersize = 7, label = 'DMRG: 6 HF')
...
def plotEntropyENT(var, val, err_val, variableName, var1, val1, val2, font, fontlegend):
	plt.errorbar(var, val, yerr=err_val, color='red', ls='-',
				 linewidth=1,  marker="o", markersize=7, label='PIGS')
	if (val1[0] != 0.0):
		plt.plot(var1, val1, linestyle='--', linewidth=2,
				 color='blue', marker="v", markersize=7, label='MM')

	if (variableName == "tau"):
		label_xtics = [0.00]
		plt.xlim(0, 0.0201)
		if (val2 != 0.0):
			plt.axhline(y=val2, color='black', lw=2.0,
						linestyle='-', label='ED')
		plt.xticks(np.arange(0.0, 0.021, step=0.005),
				   fontsize=fontlegend, rotation=0)
		plt.ylim(0.0399, 0.11001)
		plt.yticks(np.arange(0.04, 0.11, step=0.02),
				   fontsize=fontlegend, rotation=0)
	else:
		plt.xlim(0.0195, 0.12001)
		plt.xticks(np.arange(0.02, 0.13, step=0.02),
				   fontsize=fontlegend, rotation=0)
		plt.ylim(0.02499, 0.06001)
		plt.yticks(np.arange(0.025, 0.060, step=0.01),
				   fontsize=fontlegend, rotation=0)
		...
		for TypeCorr in TypeCorrList:
			...
			for RefPoint in RefPointList:
				...
				if (TypeCorr == "TotalCorr"):
					...
					if Text1:
						plt.text(textpositionx-(4.75*deltax),
								 textpositiony+(5.5*deltay), Text1, fontsize=font)

					if Text2:
						plt.text(textpositionx+(3.5*deltax), textpositiony +
								 5.5*deltay, r'$g = $'+Text2, fontsize=font)
			...
			plt.xticks(fontsize=font, rotation=0)
			plt.yticks(fontsize=font, rotation=0)
			plt.subplots_adjust(top=0.96, bottom=0.23,
								left=0.30, right=0.95, hspace=0.6, wspace=1.0)
...
def GetFitEnergy(val1, val2, val3, variableName):
	initialParameters = np.array([-7000.0, 200000.0])
	lowerBounds = (-10000.0, 0.0)
	upperBounds = (10000.0, 200000.0)
	parameterBounds = [lowerBounds, upperBounds]

	popt, pcov = curve_fit(fitFunc, val1, val2, sigma=val3, p0=initialParameters)
	perr = np.sqrt(np.diag(pcov))

	t = np.linspace(0, np.max(val1), 10)
	fit = fitFunc(t, *popt)
	if (variableName == "tau"):
		return t, fit, perr[0]
	else:
...
def PlotLabel(Text1, Text2, font, xmin, xmax, ymin, ymax, variableName, parameter, numbmolecules, molecule, Rpt, dipolemoment):
	...
	if Text1:
		plt.text(textpositionx-(4.45*deltax), textpositiony +
				 (5.0*deltay), Text1, fontsize=font)
		#plt.text(textpositionx-(4.70*deltax), textpositiony+(5.5*deltay), Text1, fontsize=font)

	if Text2:
		plt.text(textpositionx+(0.0*deltax), textpositiony +
				 0*deltay, r'$g = '+Text2+'$', fontsize=font)
...
def FigureChemicalPotentialPIGS(TypeCal, molecule_rot, TransMove, RotMove, variableName, Rpt, dipolemoment, parameterName, parameter, numbblocks, numbpass, numbmolecules, molecule, ENT_TYPE, preskip, postskip, extra_file_name, src_dir, particleA, beadsRef):
	...
	print(ydata)
	#
	plt.plot(xdata, ydata, linestyle='--', color='r',
			 label='Diagonalization', marker='v', lw=2)
	plt.errorbar(NumbRotors1, TotalEnergy1, yerr=Error1,
				 color='b', ls='-', label='PIGS', linewidth=2)
	plt.subplots_adjust(top=0.95, bottom=0.15, left=0.20,
						right=0.98, hspace=0.6, wspace=1.0)
...
#
		...
		plt.subplots_adjust(top=0.95, bottom=0.15, left=0.09,
							right=0.98, hspace=0.0, wspace=0.4)
		plt.legend(bbox_to_anchor=(0.78, 0.75), loc=2,
				   borderaxespad=1., shadow=True, fontsize=fontlegend)
		plt.savefig(outfile, dpi=200, format='eps')
...
#
		...
		plt.subplots_adjust(top=0.96, bottom=0.15, left=0.14,
							right=0.95, hspace=0.0, wspace=0.4)
		plt.legend(bbox_to_anchor=(0.75, 0.55), loc=2,
				   borderaxespad=1., shadow=True, fontsize=fontlegend)
		plt.savefig(outfile, dpi=200, format='eps')
...
#
	...
	for numbmolecules in nn:
		...
		for gFact in gFactorList:
			...
			if (numbmolecules <= 32):
				EntropyFit, ErrorFit = GetFitPurity(
					FileToBePlotEntropy, numbmolecules, "g", gFact)
...
#
	...
	plt.subplots_adjust(top=0.98, bottom=0.12, left=0.13, right=0.97, hspace=0.0, wspace=0.0)
...
#
	...
	plt.subplots_adjust(top=0.98, bottom=0.14, left=0.12, right=0.99, hspace=0.0, wspace=0.0)
	if (numbmolecules == 2):
		plt.text((xmin+(xmax-xmin)*0.4), ymax-(ymax-ymin)*0.15, r'$N =  $'+' '+str(numbmolecules), fontsize=font)
		plt.legend(bbox_to_anchor=(0.65, 0.75), loc=2, borderaxespad=1., shadow=True, fontsize=0.6*font)
	...
	if (numbmolecules == 16):
		...
		plt.legend(bbox_to_anchor=(0.65, 0.25), loc=2, borderaxespad=1., shadow=True, fontsize=0.6*font)
		plt.xlim(-0.003,0.206)
		plt.ylim(0.0,0.72)
	if (numbmolecules == 32):
...
#
	...
	yboxsize = {2: 0.7, 4: 0.7, 8: 0.5, 16: 0.5, 32: 0.5}
...
#
	xmaxlim = {2: 0.00701, 4: 0.0103, 8: 0.021, 16: 0.021, 32: 0.021}
	xminlim = {2: -0.0001, 4: -0.0001, 8: -0.001, 16: -0.001, 32: -0.001}
	ymaxlim = {2: 0.0921, 4: 0.16, 8: 0.4, 16: 0.1, 32: 0.5}
	yminlim = {2: 0.0698, 4: 0.115, 8: 0.1, 16: 0.02, 32: 0.1}
	plt.xlim(xminlim[numbmolecules], xmaxlim[numbmolecules])
	plt.ylim(yminlim[numbmolecules], ymaxlim[numbmolecules])
	ymin, ymax = plt.ylim()
	xmin, xmax = plt.xlim()
	plt.text((xmin+(xmax-xmin)*0.35),ymax-(ymax-ymin)*0.07,r'$N = \ $'+str(numbmolecules)+';'+r'$ \ g = \ $'+str(gfact),fontsize=font)
	plt.text((xmin+(xmax-xmin)*0.01),ymax-(ymax-ymin)*0.07,TextLabel[numbmolecules],fontsize=font)

	plt.subplots_adjust(top=0.98, bottom=0.14, left=0.14, right=0.98, hspace=0.0, wspace=0.0)
...
# DMRG section ends here
	...
	if (purpose == "article"):
		plt.subplots_adjust(top=0.98, bottom=0.12, left=0.13, right=0.98, hspace=0.0, wspace=0.0)
		plt.legend(bbox_to_anchor=(xboxsize[numbmolecules], yboxsize[numbmolecules]), loc=2, borderaxespad=1., shadow=True, fontsize=0.6*font)
	elif (purpose == "ppt"):
...
# DMRG section ends here
	...
	if (purpose == "article"):
		plt.subplots_adjust(top=0.97, bottom=0.12, left=0.13, right=0.97, hspace=0.0, wspace=0.0)
		plt.legend(bbox_to_anchor=(xboxsize[numbmolecules],yboxsize[numbmolecules]),loc=2,borderaxespad=1.,shadow=True,fontsize=0.6*font)
	if (purpose == "ppt"):
		plt.subplots_adjust(top=0.97, bottom=0.18, left=0.18, right=0.96, hspace=0.0, wspace=0.0)
		plt.legend(bbox_to_anchor=(xboxsize[numbmolecules],yboxsize[numbmolecules]),loc=2,borderaxespad=1.,shadow=True,fontsize=0.6*font)
...
def GetFigureEnergy_vs_R(TypeCal,molecule_rot,TransMove,RotMove,variableName,RList,gfact,dipolemoment,parameterName,parameter,numbblocks,numbpass,numbmolecules,molecule,preskip,postskip,extra_file_name,src_dir,TypePlot,purpose):
	ENT_TYPE=""
	particleA=1
	...
	plt.xlabel(r'$R \ \mathrm{\AA}$',fontsize=font,family='serif',labelpad=10)
	plt.ylabel(r'$E_{0} \ \mathrm{(Kelvin)}$',fontsize=font,family='serif',labelpad=10)

	# Plotting of fitting data
	plt.errorbar(RList, energy_fit, yerr=energy_fit_error, color='black', ls='-', linewidth=1,  marker='o', markersize=8, capsize=2, capthick=1, ecolor='black', label="PIGS-Fit")
	...
	textpositionx = xmin+midpointx-0.25*midpointx
	textpositiony = ymin+midpointy
	plt.yticks(fontsize=font, rotation=0, family='serif')
	plt.xticks(fontsize=font, rotation=0, family='serif')
	plt.xlim(2.0,xmax)

	plt.subplots_adjust(top=0.98,bottom=0.15,left=0.22,right=0.99,hspace=0.0,wspace=0.0)
	plt.legend(numpoints=1,loc=('upper right'),fontsize=font*0.5)
...
def GetFigureOrderParam_vs_R(TypeCal,molecule_rot,TransMove,RotMove,variableName,RList,gfact,dipolemoment,parameterName,parameter,numbblocks,numbpass,numbmolecules,molecule,preskip,postskip,extra_file_name,src_dir,TypePlot,purpose):
	...
	for Rpt in RList:
		...
		#print(FileToBePlotEnergy)
		beads_skip_header_final, beads_skip_footer_final = 0, 0#GetKeyIndices(numbmolecules,molecule_rot,Rpt,FileToBePlotEnergy)	
	...
	plt.xlabel(r'$R \ \mathrm{\AA}$',fontsize=font,family='serif',labelpad=10)
	plt.ylabel(r'$cicj$',fontsize=font,family='serif',labelpad=10)

	# Plotting of fitting data
	plt.errorbar(RList, fx, yerr=fx_err, color='black', ls='-', linewidth=1,  marker='o', markersize=8, capsize=2, capthick=1, ecolor='black', label="PIGS-Fit")
...
def FigureEnergyPIGS(TypeCal,molecule_rot,TransMove,RotMove,variableName,Rpt,gfact,dipolemoment,parameterName,parameter,numbblocks,numbpass,numbmolecules,molecule,ENT_TYPE,preskip,postskip,extra_file_name,src_dir,particleA,TypePlot,beadsRef,purpose):
	FilePlotName=support.GetFileNamePlot(TypeCal,molecule_rot,TransMove,RotMove,variableName,float(Rpt),gfact,dipolemoment,parameterName,parameter,numbblocks,numbpass,numbmolecules,molecule,ENT_TYPE,preskip,postskip,extra_file_name,src_dir,particleA,beadsRef)

	FilePlotEnergy = FilePlotName.SaveEnergy+".pdf"
	outfileEnergy = FilePlotName.SaveEnergy
	call(["rm", FilePlotEnergy])

	FileToBePlotEnergy = FilePlotName.SaveEnergy+".txt"
	if (molecule_rot == "H2O"):
		if (numbmolecules == 1):
			energyED = "/Users/tsahoo/ResultsOfPIGS/exact-energies-of-H2O/ground-state-energies-Field-Strength-" + \
				str(dipolemoment)+"Kinv-N1-Jmax-10-grid-30.txt"
			FileToBePlotED = energyED
		elif (numbmolecules == 2):
	...
	print(os.path.exists(FileToBePlotED))
	if (os.path.exists(FileToBePlotED)):
		if (molecule_rot == "HF"):
			FileToBePlotMM = FilePlotName.SaveEnergyMM+".txt"
			beads2, energy2 = genfromtxt(FileToBePlotMM, unpack=True, usecols=[
										 0, 1], skip_header=0, skip_footer=0)
			print(energy2)
		...
		if (molecule_rot == "H2O"):
			energy2 = 0.0
			energy3 = loadtxt(FileToBePlotED, unpack=True, usecols=[0])
	...
	'''
	k2kjmole=0.008314410016255453
	valkl=1.0/valTau
	valkj1=valRotEnergy*k2kjmole
	valkj2=valPotEnergy*k2kjmole
	valkj3=valTotalEnergy*k2kjmole
	errkj1=errorRotEnergy*k2kjmole
	errkj2=errorPotEnergy*k2kjmole
	errkj3=errorTotalEnergy*k2kjmole
	np.savetxt('pimc11.txt', np.transpose([valkl, valkj1, valkj2, valkj3, errkj1, errkj2, errkj3]), fmt='%14.6f', delimiter='  &  ', header=': Temperature,  Rot Eneergy, Pot Energy, Tot Energy, Errors: Rot, Pot, Tot', comments='# Units: KJ/mole ')
	'''
	...
	PlotEnergyPIGS(plotExtra, font, valTau, valTotalEnergy, errorTotalEnergy,
				   variableName, YLabel, valTau, energy2, energy3, Rpt)
	plt.savefig(outfileEnergy+"-tot.pdf", format='pdf')

	#YLabel = "Potential"
	#fig		  = plt.figure(figsize=(8, 6),dpi=100)
	#PlotEnergyPIGS(plotExtra, font, valTau,valPotEnergy,errorPotEnergy,variableName,YLabel,var2,energy2,energy3,Rpt)
	#plt.savefig(outfileEnergy+"-pot.pdf", format = 'pdf')

	#YLabel = "Rotational"
	#fig		  = plt.figure(figsize=(8, 6),dpi=100)
	#PlotEnergyPIGS(plotExtra, font, valTau,valRotEnergy,errorRotEnergy,variableName,YLabel,var2,energy2,energy3,Rpt)
	#plt.savefig(outfileEnergy+"-rot.pdf", format = 'pdf')

	plt.show()


def PlotEnergyPIGS(plotExtra, font, val1, val2, val3, variableName, YLabel, var2, energy2, energy3, Rpt):
	'''
					if (plotExtra):	
							#Plotting of exact energy
							plt.axhline(y=energy3, color='red', ls='--', lw=1, label = 'ED')
							#plt.plot(var2, energy2, color = 'green', ls = '--', label = 'MM', marker = "o", linewidth=2)
	'''

	if (variableName == "beta"):
		plt.xlabel(r'$\beta  \	\mathrm{(K^{-1})}$',
				   fontsize=font, family='serif', labelpad=10)

		'''
		plt.xlabel(r'$\mathrm{Temperature \ (K)}$', fontsize = font, family='serif', labelpad=10)

		# Unit conversion
		val1 = 1.0/val1
		val2 = val2*0.008314410016255453 #1K=0.008314410016255453KJ/mole
		val3 = val3*0.008314410016255453 #1K=0.008314410016255453KJ/mole
		'''
		if (YLabel == "Total"):
			#plt.ylabel(r'$\mathrm{Total \ Energy \ (KJ/mole)}$', fontsize=font, family='serif', labelpad=10)
			plt.ylabel(r'$\mathrm{Total \ Energy \ (K)}$',
					   fontsize=font, family='serif', labelpad=10)
			plt.errorbar(val1, val2, yerr=val3, color='blue', ls='--', linewidth=1, marker='o',
						 markersize=8, capsize=4, capthick=1, ecolor='blue', label='R='+str(Rpt)+r'\AA')
			if (plotExtra):
				plt.plot(var2, energy2, color='green', ls='--',
						 label='MM', marker="o", linewidth=2)

		if (YLabel == "Potential"):
			plt.ylabel(r'$\mathrm{Potential \ Energy \ (KJ/mole)}$',
					   fontsize=font, family='serif', labelpad=10)
			plt.errorbar(val1, val2, yerr=val3, color='red', ls='-.', linewidth=1, marker='s',
						 markersize=8, capsize=4, capthick=1, ecolor='red', label=YLabel+' Energy')

		if (YLabel == "Rotational"):
			plt.ylabel(r'$\mathrm{Rotational \ Energy \ (KJ/mole)}$',
					   fontsize=font, family='serif', labelpad=10)
			plt.errorbar(val1, val2, yerr=val3, color='g', ls='-', linewidth=1, marker='^',
						 markersize=8, capsize=4, capthick=1, ecolor='g', label=YLabel+' Energy')

	if (variableName == "tau"):
		plt.xlabel(r'$\tau \ \mathrm{(Kelvin^{-1})}$',
				   fontsize=font, family='serif', labelpad=10)

		if (YLabel == "Total"):
			plt.ylabel(r'$E_{0} \ \mathrm{(Kelvin)}$',
					   fontsize=font, family='serif', labelpad=10)
			# Plotting of raw data
			plt.errorbar(val1, val2, yerr=val3, color='blue', ls='None', linewidth=1,
						 marker='o', markersize=8, capsize=4, capthick=1, ecolor='blue', label='PIGS')

			# Fitting of PIGS data
			tvar, EnergyFitPlot, ErrorFitPlot = GetFitEnergy(
				val1, val2, val3, variableName)
			print(tvar)
			print(val2)
			#print(EnergyFitPlot)
			print(ErrorFitPlot)
			...
			# Plotting of fitting data
			plt.plot(tvar, EnergyFitPlot, color='black', ls='-', linewidth=1)
			plt.errorbar(tvar[0], EnergyFitPlot[0], yerr=ErrorFitPlot, color='black', ls='None',
						 linewidth=1,  marker='s', markersize=8, capsize=4, capthick=1, ecolor='black', label="Fit")
	...
	if (variableName == "tau"):
		plt.xticks(fontsize=font, rotation=45, family='serif')
		plt.text((xmin+(xmax-xmin)*0.4), ymax-(ymax-ymin) *
				 0.1, r'$R=\ $'+str(Rpt)+'\AA', fontsize=font)
	else:
		plt.xticks(fontsize=font, rotation=0, family='serif')
	#plt.xticks(np.arange(0, 1.001, step=0.2))
	#plt.yticks(np.arange(0, 0.40, step=0.1))

	plt.subplots_adjust(top=0.97, bottom=0.25, left=0.23,
						right=0.99, hspace=0.0, wspace=0.0)
	#plt.legend(bbox_to_anchor=(0.75, 0.80), loc=2, borderaxespad=1., shadow=True, fontsize = font/2)
	plt.legend(numpoints=1, loc=('center right'), fontsize=font*0.5)
