...
def	FigureEnergyPIGS(TypeCal, molecule_rot, TransMove, RotMove, variableName, Rpt, gfact, dipolemoment, parameterName, parameter, numbblocks, numbpass, numbmolecules, molecule, ENT_TYPE, preskip, postskip, extra_file_name, src_dir, particleA, TypePlot, beadsRef, purpose):
	...
	valTau, valRotEnergy, valPotEnergy, valTotalEnergy, errorRotEnergy, errorPotEnergy, errorTotalEnergy = genfromtxt(FileToBePlotEnergy, unpack=True, usecols=[1, 3, 4, 5, 7, 8, 9], skip_header=0, skip_footer=1)
	...
	plt.savefig(outfileEnergy, format = 'pdf')
...
def PlotEnergyPIGS(plotExtra, font,val1,val2,val3,variableName,YLabel,var2,energy2,energy3):
	if (variableName == "tau"):
		plt.xlabel(r'$\tau \  \mathrm{(K^{-1})}$', fontsize = font, family='serif', labelpad=10)
	if (variableName == "beta"):
		plt.xlabel(r'$\beta  \  \mathrm{(K^{-1})}$', fontsize = font, family='serif', labelpad=10)
		
	if (YLabel == "Total"):
		plt.ylabel(r'$E_{0} \ \mathrm{(K)}$', fontsize=font, family='serif', labelpad=10)
		if (variableName == "tau"):
			plotfitting(val1, val2, val3)
			plt.errorbar(val1, val2, yerr = val3, color = 'blue', ls = '', marker='o',markersize=10,label = 'PIGS', linewidth=2)
			if (plotExtra):	
		...
		if (variableName == "beta"):
			...
			if (plotExtra):	
				plt.plot(var2, energy2, color = 'green', ls = '--', label = 'MM', marker = "o", linewidth=2)
	if (YLabel == "Potential"):
		plt.ylabel(r'$V_{0}$ (K)', fontsize = font, family='serif', labelpad=10)
		plt.errorbar(val1, val2, yerr = val3, color = 'b', ls = '-', label = 'PIGS', linewidth=2)

	if (YLabel == "Rotational"):
		plt.ylabel(r'$K_{0}^{Rot}$ (K)', fontsize = font, family='serif',labelpad=10)
		plt.errorbar(val1, val2, yerr = val3, color = 'b', ls = '-', label = 'PIGS', linewidth=1)
	...
	plt.subplots_adjust(top=0.99, bottom=0.17, left=0.19, right=0.99, hspace=0.0, wspace=0.0)
	plt.legend(bbox_to_anchor=(0.75, 0.99), loc=2, borderaxespad=1., shadow=True, fontsize = font/2)
